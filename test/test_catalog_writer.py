...
def test_merge_catalog_entry_new_has_correct_id() -> None:
    ...
    entry = _merge_catalog_entry(None, _manifest(), "github", "abseil", "abseil-cpp", "vcpkg")
...
def test_merge_catalog_entry_source_type_matches_vcs_host() -> None:
    ...
    github_entry = _merge_catalog_entry(None, _manifest(), "github", "abseil", "abseil-cpp", "vcpkg")
...
def test_merge_catalog_entry_adds_version_tag() -> None:
    ...
    entry = _merge_catalog_entry(None, _manifest(version="20240116.2"), "github", "abseil", "abseil-cpp", "vcpkg")
...
def test_merge_catalog_entry_no_duplicate_version_tag() -> None:
    ...
    existing["tags"] = [{"name": "20240116.2", "is_tag": True, "commit_sha": None, "date": None}]
...
def test_merge_catalog_entry_merges_source_labels() -> None:
    ...
    entry = _merge_catalog_entry(existing, _manifest(), "github", "abseil", "abseil-cpp", "vcpkg")
...
def test_merge_catalog_entry_no_version_no_tag_added() -> None:
    ...
    entry = _merge_catalog_entry(None, _manifest(version=None), "github", "abseil", "abseil-cpp", "vcpkg")
...
def test_merge_catalog_entry_v_prefix_tag_not_duplicated() -> None:
    ...
    existing = _existing_catalog_entry()
    existing["tags"] = [{"name": "v1.2.3", "is_tag": True, "commit_sha": None, "date": None}]
    entry = _merge_catalog_entry(existing, _manifest(version="1.2.3"), "github", "abseil", "abseil-cpp", "vcpkg")
...
def test_generate_readme_contains_package_name() -> None:
    ...
    assert "abseil-cpp" in _generate_readme(_manifest(), "abseil-cpp", "https://github.com/abseil/abseil-cpp")


def test_generate_readme_contains_description() -> None:
    """Package description appears in the generated README."""
    assert "Abseil C++ libraries" in _generate_readme(_manifest(), "abseil-cpp", "https://github.com/abseil/abseil-cpp")
...
def test_generate_readme_omits_tag_when_no_version() -> None:
    ...
    readme = _generate_readme(_manifest(version=None), "abseil-cpp", "https://github.com/abseil/abseil-cpp")
...
_FULL_SHA = "a" * 40  # realistic 40-char hex SHA
...
def test_fetch_upstream_tags_returns_tag_entries() -> None:
    ...
    with patch("dfetch_hub.catalog.writer.GitRemote._ls_remote", return_value=ls_remote):
        tags = _fetch_upstream_tags("https://github.com/owner/repo")

    tag_names = {t["name"] for t in tags}
    assert tag_names == {"v1.0.0", "v2.0.0"}, "branch ref must be excluded"
...
def test_fetch_upstream_tags_commit_sha_is_full_length() -> None:
    """commit_sha must be the full 40-character SHA, not a shortened form."""
...
def test_fetch_upstream_tags_is_tag_true() -> None:
    """Every entry returned has is_tag set to True."""
    ls_remote = {"refs/tags/v1.0.0": _FULL_SHA}
    with patch("dfetch_hub.catalog.writer.GitRemote._ls_remote", return_value=ls_remote):
        tags = _fetch_upstream_tags("https://github.com/owner/repo")
...
def test_fetch_upstream_tags_returns_empty_on_error() -> None:
    ...
    with patch(
        "dfetch_hub.catalog.writer.GitRemote._ls_remote",
...
def test_merge_detail_new_sets_org_and_repo() -> None:
    ...
    with patch("dfetch_hub.catalog.writer._fetch_upstream_tags", return_value=[]):
        detail = _merge_detail(None, _manifest(), "abseil", "abseil-cpp", "vcpkg", "vcpkg", "ports")
...
def test_merge_detail_updates_existing_catalog_source() -> None:
    ...
    with patch("dfetch_hub.catalog.writer._fetch_upstream_tags", return_value=[]):
        detail = _merge_detail(existing, m, "abseil", "abseil-cpp", "vcpkg", "vcpkg", "ports")
...
def test_merge_detail_appends_new_catalog_source() -> None:
    ...
    m = _manifest()
    with patch("dfetch_hub.catalog.writer._fetch_upstream_tags", return_value=[]):
        detail = _merge_detail(existing, m, "abseil", "abseil-cpp", "conan", "conan", "recipes")
...
def test_merge_detail_urls_propagated_from_manifest() -> None:
    """urls from the manifest are written into the detail JSON."""
    m = BaseManifest(
        entry_name="abseil",
...
def test_merge_detail_urls_empty_when_manifest_has_no_urls() -> None:
    """urls field in the detail JSON is an empty dict when the manifest carries none."""
    with patch("dfetch_hub.catalog.writer._fetch_upstream_tags", return_value=[]):
        detail = _merge_detail(None, _manifest(), "abseil", "abseil-cpp", "vcpkg", "vcpkg", "ports")
    assert detail["urls"] == {}
...
def test_merge_detail_urls_merged_across_sources() -> None:
    ...
    existing = _existing_detail()
    existing["urls"] = {"Homepage": "https://github.com/abseil/abseil-cpp"}
    m = BaseManifest(
        entry_name="abseil",
        package_name="abseil-cpp",
        description="desc",
        ...
        license=None,
        version=None,
        urls={"Source": "https://github.com/conan-io/conan-center-index"},
    )
    with patch("dfetch_hub.catalog.writer._fetch_upstream_tags", return_value=[]):
        detail = _merge_detail(existing, m, "abseil", "abseil-cpp", "conan", "conan", "recipes")
    assert "Homepage" in detail["urls"]
    assert "Source" in detail["urls"]
